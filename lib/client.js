window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function tokenReadOnlyHint(ref) {
      ...
      return "令牌由启动 DSH 的环境变量 " + name + " 提供（只读），面板已直接使用，无需在此保存。"
        + "如需改由设置页管理，请先移除该环境变量并重启 DSH；Windows 可在 PowerShell 执行 "
        + "[Environment]::SetEnvironmentVariable('" + name + "', $null, 'User') 后重开终端。";
    }
    ...
    function createJiraSettingsCard(scope, credentials) {
      function readTokenInfo() {
        return credentials.describe(TOKEN_REFS).then(function (res) {
          ...
          for (var i = 0; i < TOKEN_REFS.length; i++) {
            var ref = TOKEN_REFS[i];
            var info = res.value[ref];
            // 命中的第一个引用即生效来源；writable 为 false 表示只读（由启动环境提供），写不进去。
            if (info && info.configured) {
              return { configured: true, writable: info.writable !== false, ref: ref };
            }
          }
          var primary = res.value[TOKEN_REF];
          return { configured: false, writable: primary ? primary.writable !== false : true, ref: TOKEN_REF };
        }).catch(function () {
      ...
      return function JiraSettingsCard() {
        ...
        // 令牌可能来自启动环境（只读）：此时禁用输入并说明原因，而不是等到保存才报 DSH 的英文错误。
        var tokenWritable = !tokenInfo || tokenInfo.writable !== false;
        var tokenReadOnly = !!(tokenInfo && tokenInfo.configured && tokenInfo.writable === false);
        ...
        function save() {
          ...
          if (token && !tokenWritable) {
            setBusy(false);
            setFailed(tokenReadOnlyHint(tokenInfo && tokenInfo.ref));
            return;
          }
          if (token) {
        ...
        var tokenHint = tokenInfo === null
          ? "正在读取令牌状态…"
          : (tokenReadOnly
            ? tokenReadOnlyHint(tokenInfo.ref)
        ...
        return h("li", { className: "jt-set-card" },
          ...
          h("div", { className: "jt-field" },
            ...
            h("input", {
              ...
              type: "password",
              value: tokenDraft,
              placeholder: tokenReadOnly ? "由环境变量提供（只读）" : (tokenInfo && tokenInfo.configured ? "已配置，留空保持不变" : "粘贴令牌"),
              autoComplete: "off",
              disabled: !writable || busy || !tokenWritable,
          ...
          h("div", { className: "jt-buttons" },
            ...
            h("button", { className: "jt-btn jt-btn-primary", disabled: !dirty || busy || !writable || ((tokenDraft || "").trim().length > 0 && !tokenWritable), onClick: save }, busy ? "保存中…" : "保存")
          )
        );
      };
    }
